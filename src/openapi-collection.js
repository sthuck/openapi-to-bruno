...
/**
 *
 * @param {string} file file path to read, can be either json or yaml
 * @returns
 */
const readFile = (file) => {
  if (file.endsWith('.yaml') || file.endsWith('.yml')) {
    return readFileNode(file, 'utf8').then(jsyaml.load)
  } else if (file.endsWith('.json')) {
    return readFileNode(file, 'utf8').then(JSON.parse)
  }
  return Promise.reject(
    new Error('Invalid file format. Only JSON and YAML files are supported.'),
  )
}
...
const getDefaultUrl = (serverObject) => {
  ...
  if (serverObject.variables) {
    each(serverObject.variables, (variable, variableName) => {
      let sub =
        variable.default ||
        (variable.enum ? variable.enum[0] : `{{${variableName}}}`)
...
const parseOpenApiCollection = (data) => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      console.error(err)
      reject(
        new Error('An error occurred while parsing the OpenAPI collection'),
      )
    }
  })
}

const importOpenApiCollection = (filePath) => {
  return new Promise((resolve, reject) => {
...
export default importOpenApiCollection
